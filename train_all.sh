...
python train_stage1_zip.py --config config.yaml > logsb/stage1.log 2>&1
echo "✅ Stage 1 completato!"

echo "🚀 Avvio Stage 2 (P2R)..."
# Resume attivo per sicurezza
python train_stage2_p2r.py --config config.yaml > logsb/stage2.log 2>&1
echo "✅ Stage 2 completato!"

echo "🚀 Avvio Stage 3 (JOINT)..."
python train_stage3_joint.py --config config.yaml > logsb/stage3.log 2>&1
echo "✅ Stage 3 completato!"

echo "🚀 Avvio Valutazioni..."
python evaluate_stage1.py > logsb/ev_stage1.log 2>&1
python evaluate_stage2.py > logsb/ev_stage2.log 2>&1
python evaluate_stage3.py > logsb/ev_stage3.log 2>&1
python visualize_gating.py > logsb/visualize_gating.log 2>&1
