#!/usr/bin/env python3
"""
Stage 2 V8.2 - P2R Training con Multi-Scale Loss + Augmentation Potenziata

FIX V8.2:
- Aggiunto debug per salvataggio checkpoint
- Try/except attorno ai salvataggi
- Flush esplicito dopo print
- Verifica esistenza file dopo salvataggio

MODIFICHE DA V7:
1. Multi-scale loss con scales [1, 2, 4]
2. Data augmentation potenziata
3. Warmup + gradient clipping
...
# =============================================================================
# CHECKPOINT UTILS - FIX V8.2
# =============================================================================

def save_checkpoint(state, filepath, description="checkpoint"):
    """Salva checkpoint con verifica e debug."""
    try:
...
def main():
    ...
    parser.add_argument('--stage1-ckpt', type=str, default=None, help='Path specifico checkpoint Stage 1')
    ...
    epochs = p2r_cfg.get('EPOCHS', 4000)
    patience = p2r_cfg.get('EARLY_STOPPING_PATIENCE', 1200)
    ...
    print("=" * 60)
    print("🚀 Stage 2 V8.2 - Multi-Scale Loss (FIX CHECKPOINT)")
    print("=" * 60)
    ...
    # Se specificato da CLI, usa quello
    if args.stage1_ckpt:
        stage1_candidates = [args.stage1_ckpt]
    else:
        stage1_candidates = [
            os.path.join(output_dir, "stage1_best_acc.pth"),   # Preferito
    ...
    if not stage1_loaded:
        ...
        print(f"   Cercato in: {stage1_candidates}")
    ...
    if not args.no_resume and os.path.isfile(checkpoint_path):
        print(f"\n🔄 Trovato checkpoint Stage 2: {checkpoint_path}")
        try:
            ...
            print(f"⚠️ Errore resume: {e}, ripartendo da zero")
            start_epoch = 1
            best_mae = float('inf')
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # Salva checkpoint per resume (ogni 10 epoche per non rallentare troppo)
        if epoch % 10 == 0:
            save_checkpoint(
                {
                    'epoch': epoch,
    ...
    if best_mae < 65:
        ...
        print(f"   ✅ Buon risultato")
    else:
        print(f"   ⚠️ Potrebbe servire tuning")
    
    print("=" * 60)
    print(f"\n📌 Prossimo step: python train_stage3.py --config {args.config}")
